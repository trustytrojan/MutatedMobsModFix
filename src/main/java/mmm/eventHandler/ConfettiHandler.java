...
public class ConfettiHandler
{
    ...
    public void creeperExplodeEvent(final LivingEvent.LivingUpdateEvent event) {
        ...
        if (creeper != null && creeper.getCreeperState() == 1) {
            final int ignitedTime = (int)ReflectionHelper.getPrivateValue(EntityHalfCreeper.class, creeper, ConfettiHandler.TIME_SINCE_IGNITED);
            final int fuseTime = (int)ReflectionHelper.getPrivateValue(EntityHalfCreeper.class, creeper, ConfettiHandler.FUSE_TIME);
