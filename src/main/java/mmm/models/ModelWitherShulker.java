...
public class ModelWitherShulker extends ModelBase
{
    ...
    public void setRotationAngles(final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scaleFactor, final Entity entityIn) {
        final EntityWitherShulker EntityWitherShulker = (EntityWitherShulker)entityIn;
        final float f = ageInTicks - EntityWitherShulker.ticksExisted;
        final float f2 = (0.5f + EntityWitherShulker.getClientPeekAmount(f)) * 3.1415927f;
        ...
        if (EntityWitherShulker.getClientPeekAmount(f) > 0.3f) {
